...
public:
    DebugLogger(bool verbose = true)
        ...
        if (verbose_mode_) {
            ...
            if (!verbose_log_file_.is_open()) {
                std::cerr << Color::RED << "[WARNING] Could not open verbose log file" << Color::RESET << "\n";
            } else {
    ...
    void log_json(const std::string& color, const std::string& category,
                  const std::string& label, const json& data) {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    void increment_hook_counter(const std::string& hook_type) {
        if (hook_type == HookEvent::PreToolUse) pre_tool_use_count_++;
        else if (hook_type == HookEvent::PostToolUse) post_tool_use_count_++;
        else if (hook_type == HookEvent::UserPromptSubmit) user_prompt_submit_count_++;
        else if (hook_type == HookEvent::Stop) stop_count_++;
        else if (hook_type == HookEvent::SubagentStop) subagent_stop_count_++;
        else if (hook_type == HookEvent::PreCompact) pre_compact_count_++;
    }
    ...
    void increment_permission_request() { permission_request_count_++; }
    void increment_stderr_line() { stderr_line_count_++; }
    void increment_message() { message_count_++; }
    void increment_assistant_message() { assistant_message_count_++; }
    void increment_result_message() { result_message_count_++; }
    void increment_stream_event() { stream_event_count_++; }
    void increment_control_request() { control_request_count_++; }
    void increment_control_response() { control_response_count_++; }
    ...
    void print_statistics() {
        ...
        log(Color::CYAN, "STATS", "  Assistant messages: " + std::to_string(assistant_message_count_.load()));
        log(Color::CYAN, "STATS", "  Result messages: " + std::to_string(result_message_count_.load()));
        log(Color::CYAN, "STATS", "  Stream events: " + std::to_string(stream_event_count_.load()));
        log(Color::CYAN, "STATS", "  Control requests: " + std::to_string(control_request_count_.load()));
        log(Color::CYAN, "STATS", "  Control responses: " + std::to_string(control_response_count_.load()));

        log(Color::CYAN, "STATS", "");
        log(Color::CYAN, "STATS", "=== Hook Statistics ===");
        log(Color::CYAN, "STATS", "  PreToolUse hooks: " + std::to_string(pre_tool_use_count_.load()));
        log(Color::CYAN, "STATS", "  PostToolUse hooks: " + std::to_string(post_tool_use_count_.load()));
        log(Color::CYAN, "STATS", "  UserPromptSubmit hooks: " + std::to_string(user_prompt_submit_count_.load()));
        log(Color::CYAN, "STATS", "  Stop hooks: " + std::to_string(stop_count_.load()));
        log(Color::CYAN, "STATS", "  SubagentStop hooks: " + std::to_string(subagent_stop_count_.load()));
        log(Color::CYAN, "STATS", "  PreCompact hooks: " + std::to_string(pre_compact_count_.load()));

        log(Color::CYAN, "STATS", "");
        log(Color::CYAN, "STATS", "=== Callback Statistics ===");
        log(Color::CYAN, "STATS", "  Permission requests: " + std::to_string(permission_request_count_.load()));
...
json user_prompt_submit_hook(const json& input, const std::string& /*tool_use_id*/) {
    ...
    // Log to verbose structured log
    g_logger.log_verbose_event("hook_user_prompt_submit", {
        {"hook_input", input}
    });
...
json stop_hook(const json& input, const std::string& /*tool_use_id*/) {
    ...
    // Log to verbose structured log
    g_logger.log_verbose_event("hook_stop", {
        {"hook_input", input}
    });
...
json subagent_stop_hook(const json& input, const std::string& /*tool_use_id*/) {
    ...
    // Log to verbose structured log
    g_logger.log_verbose_event("hook_subagent_stop", {
        {"hook_input", input}
    });
...
json pre_compact_hook(const json& input, const std::string& /*tool_use_id*/) {
    ...
    // Log to verbose structured log
    g_logger.log_verbose_event("hook_pre_compact", {
        {"hook_input", input}
    });
...
PermissionResult tool_permission_callback(const std::string& tool_name,
                                         const json& input,
    ...
    // Log to verbose structured log
    g_logger.log_verbose_event("tool_permission_request", {
        {"tool_name", tool_name},
        {"input", input},
...
void stderr_callback(const std::string& line) {
    ...
    // Log to verbose structured log
    g_logger.log_verbose_event("cli_stderr", {
        {"line", line}
    });
}
...
ClaudeOptions configure_debug_options() {
    ...
    opts.hooks[HookEvent::PreToolUse] = {
        HookMatcher{std::nullopt, {pre_tool_use_hook}}
    };

    opts.hooks[HookEvent::PostToolUse] = {
        HookMatcher{std::nullopt, {post_tool_use_hook}}
    };
    ...
    opts.hooks[HookEvent::Stop] = {
        HookMatcher{std::nullopt, {stop_hook}}
    };

    opts.hooks[HookEvent::SubagentStop] = {
        HookMatcher{std::nullopt, {subagent_stop_hook}}
    };

    opts.hooks[HookEvent::PreCompact] = {
        HookMatcher{std::nullopt, {pre_compact_hook}}
    };
...
std::vector<TestPrompt> get_test_prompts() {
    return {
        ...
        {
            "Bash Command",
            "Run the command 'echo Testing Bash Integration' using bash.",
        ...
        {
            "Direct Answer",
            "What is 2+2? Just answer directly, don't use any tools.",
    ...
    for (const auto& msg : client.receive_messages()) {
        ...
        if (is_assistant_message(msg)) {
            ...
            if (!text.empty()) {
                ...
                std::string display_text = text.length() > 200
                    ? text.substr(0, 200) + "..."
                    : text;
            ...
            g_logger.log_verbose_event("message_assistant", {
                {"role", assistant.role},
            ...
            std::ostringstream oss;
            oss << "Result: " << result.subtype
                << " | Duration: " << result.duration_ms << "ms"
                << " | Turns: " << result.num_turns
                << " | Cost: $" << std::fixed << std::setprecision(4) << result.total_cost_usd();
            g_logger.log(Color::GREEN, "RESULT", oss.str());

            // Log to verbose structured log with FULL details
            g_logger.log_verbose_event("message_result", {
            ...
            g_logger.log_verbose_event("message_stream", {
                {"event", stream.event},
                {"index", stream.index},
                {"uuid", stream.uuid},
            ...
            g_logger.log_json(Color::YELLOW, "CONTROL_REQ",
                            "Control Request: " + method_str, control.request);
...
int main() {
    ...
    g_logger.log(Color::CYAN, "INFO", "This tool hooks ALL available callbacks and logs the complete IPC protocol");
    ...
    try {
        ...
        client.connect();
        g_logger.log(Color::GREEN, "CONNECT", "Connected! PID: " + std::to_string(client.get_pid()));
        ...
        g_logger.log(Color::GREEN, "SUCCESS", "  - debug_protocol_verbose.json (structured, LLM-friendly)");
        g_logger.log(Color::CYAN, "INFO", "");
