                                                ...
                                                const ConformanceOptions& options)
{
    ...
    bool has_list_summaries =
        has_optional(*probe, ConformanceOptional::ListSessionSummaries, options.skip_optional);
    bool has_delete =
    ...
    runner.run(
        "1.append_load_roundtrip", false,
        [&]
        {
            auto s = make_store();
            s->append(KEY, {e({{"uuid", "b"}, {"n", 1}}), e({{"uuid", "a"}, {"n", 2}})});
            auto loaded = s->load(KEY);
            check(loaded.has_value() && loaded->size() == 2, "expected 2 loaded entries");
            check((*loaded)[0]["uuid"] == "b" && (*loaded)[1]["uuid"] == "a", "order mismatch");
        },
        result);

    // 2. load unknown key returns nullopt
    runner.run(
        "2.load_unknown_returns_nullopt", false,
        [&]
        {
            auto s = make_store();
    ...
    runner.run(
        ...
        [&]
        {
            ...
            check((*l)[0]["uuid"] == "z" && (*l)[1]["uuid"] == "a" && (*l)[2]["uuid"] == "m" &&
                      (*l)[3]["uuid"] == "b",
                  "order");
        },
        result);
    ...
    runner.run(
        "5.subpath_independence", false,
        [&]
        {
            auto s = make_store();
            claude::SessionKey sub{KEY.project_key, KEY.session_id,
                                   std::string("subagents/agent-1")};
    ...
    runner.run(
        "7.list_sessions_basics", !has_list_sessions,
        [&]
        {
            auto s = make_store();
    ...
    runner.run(
        "8.list_sessions_excludes_subpaths", !has_list_sessions,
        [&]
        {
            auto s = make_store();
            s->append({"proj", "main", std::nullopt}, {e({{"n", 1}})});
            s->append({"proj", "main", std::string("subagents/agent-1")}, {e({{"n", 1}})});
            auto sessions = s->list_sessions("proj");
            check(sessions.size() == 1 && sessions[0].session_id == "main", "main only");
        },
        result);

    // 14. list_session_summaries
    runner.run(
        "14.list_session_summaries", !has_list_summaries,
        [&]
        {
            auto s = make_store();
            ...
            check(refolded.mtime == summ.mtime, "fold preserves prev.mtime verbatim");
            // Subagent append must NOT affect main summary.
            s->append(
                {key.project_key, key.session_id, std::string("subagents/agent-1")},
                {e({{"timestamp", "2024-01-01T00:00:09.000Z"}, {"customTitle", "subagent"}})});
            ...
            if (has_delete)
            {
                ...
                check(s->list_session_summaries("proj").empty(), "delete clears summary");
            }
        },
        result);
    ...
    runner.run(
        "9.delete_returns_nullopt", !has_delete,
        [&]
        {
            auto s = make_store();
    ...
    runner.run(
        "10.delete_cascades", !has_delete,
        [&]
        {
            auto s = make_store();
            claude::SessionKey sub1{KEY.project_key, KEY.session_id,
                                    std::string("subagents/agent-1")};
            claude::SessionKey sub2{KEY.project_key, KEY.session_id,
                                    std::string("subagents/agent-2")};
            ...
            if (has_list_subkeys)
                check(s->list_subkeys({KEY.project_key, KEY.session_id}).empty(), "subkeys gone");
        },
        result);

    // 11. delete with subpath removes only that subkey
    runner.run(
        "11.delete_subpath_only", !has_delete,
        [&]
        {
            auto s = make_store();
            claude::SessionKey sub1{KEY.project_key, KEY.session_id,
            ...
            if (has_list_subkeys)
            {
                ...
                check(sks.size() == 1 && sks[0] == "subagents/agent-2", "only agent-2 remains");
            }
        },
        result);
    ...
    runner.run(
        "12.list_subkeys_scoped", !has_list_subkeys,
        [&]
        {
            auto s = make_store();
            s->append(KEY, {e({{"n", 1}})});
            ...
            check(sks.size() == 2 && sks[0] == "subagents/agent-1" && sks[1] == "subagents/agent-2",
    ...
    runner.run(
        "13.list_subkeys_excludes_main", !has_list_subkeys,
        [&]
        {
            auto s = make_store();
            s->append(KEY, {e({{"n", 1}})});
            check(s->list_subkeys({KEY.project_key, KEY.session_id}).empty(), "no subkeys yet");
