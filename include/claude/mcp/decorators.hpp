...
//   auto echo = CLAUDE_TOOL_WITH_PARAMS("echo", "Echo a string", [](std::string s){ return s; }, "text");
//   auto handler = CLAUDE_SERVER("demo", "0.1.0", add, echo);
...
#define CLAUDE_TOOL_WITH_PARAMS(NAME, DESC, FUNC, /*param names...*/ ...) \
  ::claude::mcp::Tool(::claude::mcp::make_tool(std::string(NAME), std::string(DESC), (FUNC), std::vector<std::string>{__VA_ARGS__}))
