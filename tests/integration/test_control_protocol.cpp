...
TEST_F(ControlProtocolTest, HookCallbackApproval) {
    ...
    auto hook_callback = [&hook_called](const claude::json& input, const std::string& tool_use_id) -> claude::json {
        hook_called = true;
    ...
    for (const auto& msg : client.receive_messages()) {
        ...
        if (claude::is_result_message(msg)) break;
    }
...
TEST_F(ControlProtocolTest, ToolPermissionAllow) {
    ...
    opts.tool_permission_callback = [&permission_requested](
...
TEST_F(ControlProtocolTest, MultipleHooks) {
    ...
    auto hook_callback = [&hook_count](const claude::json& input, const std::string& tool_use_id) -> claude::json {
        hook_count++;
        return claude::json{
            {"continue", true}
        };
